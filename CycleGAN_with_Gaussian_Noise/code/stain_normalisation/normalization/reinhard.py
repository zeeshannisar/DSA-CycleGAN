...
from stain_normalisation.normalization.normalizer_abc import Normaliser
...
class ReinhardNormalizer(Normaliser):
    ...
    def write_fit(self, outputfilename):
        with h5py.File(outputfilename, 'w') as f:
            f.create_dataset("target_means", data=self.target_means, dtype='f')
            f.create_dataset("target_stds", data=self.target_stds, dtype='f')

    def read_fit(self, inputfilename):
        with h5py.File(inputfilename, 'r') as f:
            self.target_means = np.array(f.get('target_means'))
            self.target_stds = np.array(f.get('target_stds'))
