...
import numpy as np
from stain_normalisation.utils import misc_utils as mu
from stain_normalisation.utils.rj_utils import RuifrokJohnstonDeconvolution
#from stain_normalisation.normalization.macenko import MacenkoNormalizer
#from stain_normalisation.normalization.vahadane import VahadaneNormalizer
import copy
...
class Fetcher(object):
    ...
    def compute(self, I, just_stain=False, staincode=None):
        """
        ...
        zerodchannels = []
        if self.method == 'RJ':
            stain_matrix, zerodchannels = self.stain_fetcher(staincode)
        else:
        ...
        if just_stain:
            ...
            return stain_matrix, source_concentrations, zerodchannels
...
class TellezAugmentor(object):
    ...
    def fit(self, I, staincode=None, standardize_brightness=True):
        """
        ...
        if self.method == 'RJ':
            self.stain_matrix, self.source_concentrations, self.zerodchannels = self.fetcher.compute(I, staincode=staincode)
        else:
    ...
    def augment(self, new_stain_mat=False, include_background=False):
        ...
        for i in range(channels):
            if i not in self.zerodchannels:
        ...
        I_prime = np.exp(-(np.dot(source_concentrations, self.stain_matrix) - 255) * np.log(255) / 255) - 1
        I_prime = np.clip(I_prime, 0, 255).reshape(self.Ishape).astype(np.uint8)
