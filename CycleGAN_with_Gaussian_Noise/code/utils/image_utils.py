...
def read_svs_image_forced(imagePath, lod):
    ...
    if numpy.where(vecOfScales.astype(int) == scaleFactor)[0].size > 0:  #  If the scale exist
        ...
        image = skimage.transform.resize(imageLowerLod, (
            int(dimX / (scaleFactor / lowerScaleFactor)), int(dimY / (scaleFactor / lowerScaleFactor)), imageLowerLod.shape[2]), order=1, mode='reflect', clip=True, preserve_range=True, anti_aliasing=False)
...
def downsample_gt(image, target_size):
    print("image.shape: {} | target_size.shape: {}".format(image.shape, target_size))
...
def downsample_gt_image(image, repetitions):
    print("image.shape: {} | target_size.shape: {}".format(image.shape, repetitions))
