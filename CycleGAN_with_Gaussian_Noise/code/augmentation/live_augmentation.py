...
class ImageDataGenerator(object):
    ...
    def random_transform(self, x, y, j=None, staincode=None):
        ...
        for i in ord:
            ...
            #    use_elastic = random.choice([True, False])
            #else:
            #    use_elastic = True

            if self.methods[i] == 'elastic' and random.random() < 0.5:
            ...
            if self.methods[i] == 'affine' and random.random() < 0.5:
            ...
            if self.methods[i] == 'stain' and random.random() < 0.5:
...
class NumpyArrayIterator(Iterator):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        for i, j in enumerate(index_array):
            x, y = self.image_data_generator.random_transform(self.x, self.y, j=j, staincode=staincodes)
...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator, filepath=None,
        ...
        if not classes:
            ...
            for subdir in sorted(os.listdir(os.path.join(directory, 'images'))):
                if os.path.isdir(os.path.join(os.path.join(directory, 'images'), subdir)):
        ...
        i = 0
        for dirpath in (os.path.join(directory, 'images', subdir) for subdir in classes):
    ...
    def get_fit_stats(self, standardise_samples):
        ...
        for i, filename in enumerate(self.filenames):
            ...
            y = load_img(os.path.join(self.directory, 'gts', filename),
                         grayscale=True, target_size=self.gt_target_size, data_format=self.data_format).astype(numpy.uint)
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        for i, j in enumerate(index_array):
            ...
            x = load_img(os.path.join(self.directory, 'images', fname),
                           grayscale=False, target_size=self.img_target_size, data_format=self.data_format)
            #  Umesto images ovd etreba da ide gts. Zbog random sampling-ovanja nemam gts pa je ovo samo hack da bi CycleGAN mogao da se trenira.
            y = load_img(os.path.join(self.directory, 'images', fname),
                          grayscale=True, target_size=self.gt_target_size, data_format=self.data_format)

            if self.filepath:
                staincode = self.filepath.get_stain(fname)
                x, y = self.image_data_generator.random_transform(x, y, staincode=staincode)
            else:
    ...
    def _get_batches_of_transformed_samples_with_fnames(self, index_array):
    ...
    def next_with_fnames(self):
        ...
        # The transformation of images is not under thread lock
        # so it can be done in parallel
        return self._get_batches_of_transformed_samples_with_fnames(index_array)
