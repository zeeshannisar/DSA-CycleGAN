...
import warnings
from helper import forcedImageSvs
from helper.colourconv import rgb2hed
import skimage.transform
import os
from helper.multiclass_downsampling import countless
import math
import pyvips
import copy
import json
...
def image_colour_convert(img, color_mode='rgb'):
    ...
    if color_mode == 'rgb':
        ...
        return (rgb2gray(img / 255)[..., None] * 255).astype(imgdtype)


def standardise_tf(sample):
...
def normalise(sample, mean, stddev):
    sample -= mean
...
def save_image(array, savePath):
    ...
    if len(array.shape) == 3:
        img = pyvips.Image.new_from_memory(array.tobytes(order='C'), array.shape[1], array.shape[0], array.shape[2],
                                           dtype_to_format[str(array.dtype)])
    elif len(array.shape) == 2:
        img = pyvips.Image.new_from_memory(array.tobytes(order='C'), array.shape[1], array.shape[0], 1,
                                           dtype_to_format[str(array.dtype)])
    else:
...
def read_svs_image_forced(imagePath, lod):
    ...
    if numpy.where(vecOfScales.astype(int) == scaleFactor)[0].size > 0:  # If the scale exist
        level = numpy.where(vecOfScales.astype(int) == scaleFactor)[0][
            0]  # Find the indice of the given scaleFactor in the vector of scales.
        image = imageSvs.read_region((0, 0), level, imageSvs.level_dimensions[level])
        ...
        image = skimage.transform.resize(imageLowerLod, (
            int(dimX / (scaleFactor / lowerScaleFactor)), int(dimY / (scaleFactor / lowerScaleFactor)),
            imageLowerLod.shape[2]), order=1, mode='reflect', clip=True, preserve_range=True, anti_aliasing=False)
    ...
    for i, c in enumerate(classes):
        ...
        if not file_extension == '.tif' or file_extension == '.tiff':
            if not os.path.isfile(os.path.join(tempfile.gettempdir(), os.path.basename(filename) + '_' + c + '.tif')):
                convert_image(os.path.join(segmentationpath, c, image),
...
def image_colour_convert(img, color_mode='rgb'):
    ...
    if color_mode == 'rgb':
        ...
        return (rgb2grey(img / 255)[..., None] * 255).astype(imgdtype)
    elif color_mode == 'haemotoxylin':
        return (rgb2hed(img / 255)[..., [0]] * 255).astype(imgdtype)


def relabel_gt(gt, classLabels, class_definitions):
    updated_class_definitions = copy.deepcopy(class_definitions)

    oldgt = copy.deepcopy(gt)
    overlapping_labels = set()
    updatedlabels = set()
    for gtClassName in classLabels.keys():

        # get detector label
        if gtClassName in class_definitions:
            detectorlabel = class_definitions[gtClassName][0]
            updateddetectorlabel = updated_class_definitions[gtClassName][0]
        else:
            raise ValueError('Class %s does not exist in the ground truth' % (gtClassName))

        # get ground truth label for the same class
        gtLabel = classLabels[gtClassName]

        # if another ground truth class already has that label, record its name (to check that it is corrected later)
        for existingGtClassName in set(classLabels.keys()) - {gtClassName} - updatedlabels:
            if classLabels[existingGtClassName] == updateddetectorlabel:
                overlapping_labels.add(existingGtClassName)

        # switch label in ground truth for detector label
        if gtLabel != detectorlabel:
            gt[oldgt == gtLabel] = detectorlabel
            updatedlabels.add(gtClassName)
            overlapping_labels.discard(gtClassName)
            updated_class_definitions[gtClassName][0] = detectorlabel

    # check that all overlapping labels have been corrected
    if overlapping_labels:
        raise ValueError('The labels of class(es) %s were assigned to (an)other class(es) but were not reassigned' % (
            ', '.join(overlapping_labels)))

    return gt


def check_gt_validity(classLabelFilename, class_definitions):
    with open(classLabelFilename, 'r') as fp:
        classLabels = json.load(fp)

    if not set(classLabels.keys()) == set(class_definitions.keys()):
        return false

    for gtClassName in classLabels.keys():
        if not classLabels[gtClassName] == class_definitions[gtClassName][0]:
            return false
